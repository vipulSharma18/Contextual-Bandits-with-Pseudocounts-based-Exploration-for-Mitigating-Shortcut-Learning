...
class ContextualBanditWithPseudoCounts(nn.Module): 
    def __init__(self, context_length=256, hidden_size=512, num_coins=64, lambda_explore=0.8): 
        self.pseudocount_gen = ExplorationBonus(input_size=context_length, hidden_size=512, output_size=num_coins)
        self.coin_flip_maker = CoinFlipMaker(num_coins=num_coins)
        self.exploration_rate = ExponentialDecayExploration(init_value=lambda_explore, exp_decay_rate=0.02) #start at 0.8, reach 0.1 at 100th step/batch
    ...
    def forward(self, context): 
        ...
        total_reward = pred_reward + self.exploration_rate()*exploration_bonus
